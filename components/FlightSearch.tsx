...
import { PlaneTakeoff, ArrowRightLeft, ArrowUpRight, ChevronDown, Plus, Minus } from "lucide-react";
...
export function FlightSearch({ onSearch }: FlightSearchProps) {
    ...
    const handleSearch = () => {
        ...
        if (tripType !== 'one-way' && returnDate && date && returnDate < date) {
    ...
    return (
        <div className="h-auto min-h-[200px] lg:h-[340px] w-full max-w-[1500px] mx-auto bg-white rounded-[1.5rem] flex flex-col pt-4 relative z-10 font-sans shadow-xl">
            {/* Header / Trip Type Logic */}
            <div className="flex items-center justify-between border-b border-slate-100 bg-white rounded-t-[1.5rem]">
                <div className="flex px-4 md:px-10 pt-0 pb-4 bg-white relative border-b-4 border-brand text-brand">
                    <div className="flex items-center gap-2 font-bold text-[20px] md:text-[22px]">
                        <PlaneTakeoff className="w-6 h-6 md:w-8 md:h-8" strokeWidth={2.5} />
                        <span className="tracking-tight">Flights</span>
                    </div>
                </div>
                <div className="hidden sm:flex items-center text-[19px] font-semibold pr-4">
                    <Popover open={isLastSearchesOpen} onOpenChange={setIsLastSearchesOpen}>
                        <PopoverTrigger asChild>
                            <div className="flex items-center gap-1.5 px-6 py-4 cursor-pointer text-slate-400 hover:text-slate-600 transition-colors">
                            ...
                            {lastSearches.length > 0 ? (
                                <div className="flex flex-col gap-1">
                                    {lastSearches.map((search, idx) => (
                                        ...
                                        >
                                            <div className="font-bold text-slate-800 text-[15px]">{search.origin} → {search.destination}</div>
                                            <div className="text-slate-500 text-[13px]">{search.date}</div>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                    ...
                    <div className="px-6 py-4 border-l border-slate-200 cursor-pointer text-slate-400 hover:text-slate-600 transition-colors" onClick={handleReset}>
                        Reset Search
                    </div>
                </div>
            </div>

            <div className="px-4 py-4 md:px-10 md:py-8 flex flex-col gap-4 md:gap-6">
                {/* Trip Type Selector */}
                <div className="flex items-center flex-wrap gap-3 md:gap-0 md:space-x-6">
                    {(["one-way", "round-trip", "multi-city"] as const).map((type) => (
                        <label key={type} className="flex items-center space-x-2 cursor-pointer group" onClick={() => setTripType(type)}>
                            <div className={cn(
                                "w-[20px] h-[20px] rounded-full border-2 flex items-center justify-center shadow-sm",
                                tripType === type ? "border-brand" : "border-slate-300"
                            )}>
                                {tripType === type && <div className="w-3 h-3 rounded-full bg-brand" />}
                            </div>
                            <span className={cn(
                                "text-[18px] leading-none pt-0.5",
                                tripType === type ? "text-slate-800 font-bold" : "text-slate-400 font-semibold"
                            )}>
                ...
                <div className="flex flex-col lg:flex-row items-start justify-between w-full mt-2 lg:gap-2 xl:gap-6 relative" ref={searchRef}>
                    {/* Origin Input */}
                    <div className="flex flex-col flex-1 min-w-[210px] group relative h-[100px] w-full lg:w-auto">
                        <label className="text-[18px] font-bold text-slate-400 mb-2 block">Departure From</label>
                        <input
                            type="text"
                            className="bg-transparent border-none outline-none font-bold text-slate-900 tracking-tighter text-[30px] p-0 placeholder:text-slate-300 leading-none h-[34px] w-full"
                            value={originSearch}
                            onChange={(e) => {
                        ...
                        />
                        <p className="text-[15px] text-slate-500 mt-2 truncate font-medium">
                            {GLOBAL_CITIES.find(c => c.name.toLowerCase() === origin.toLowerCase() || c.code.toLowerCase() === origin.toLowerCase())?.name || origin || "Anywhere"}
                        </p>
                        <div className="absolute bottom-[-10px] left-0 w-full h-[1.5px] bg-slate-200 group-hover:bg-slate-300 hidden lg:block" />
                                    ...
                                    {GLOBAL_CITIES.filter(c =>
                                        ...
                                        >
                                            <div className="flex flex-col">
                                                <span className="font-bold text-slate-800 text-[16px]">{city.name}</span>
                                                <span className="text-slate-400 text-[14px]">{city.country}</span>
                                            </div>
                                            <span className="font-bold text-slate-300 bg-slate-100 px-2 py-1 rounded text-[13px]">{city.code}</span>
                                        </li>
                                    ))}
                                    {GLOBAL_CITIES.filter(c =>
                                        c.name.toLowerCase().includes(debouncedOriginSearch.toLowerCase()) ||
                    ...
                    {/* Enlarged Swap Button */}
                    <div className="hidden lg:flex absolute left-[19.5%] top-[30%] -translate-x-1/2 -translate-y-1/2 z-10 w-14 h-14 rounded-full border-2 border-slate-100 bg-white text-brand items-center justify-center shadow-md hover:scale-110 hover:bg-rose-50 hover:border-brand hover:shadow-lg transition-all cursor-pointer group/swap"
                        onClick={() => {
                        ...
                        <ArrowRightLeft className="w-6 h-6 transform group-hover/swap:rotate-180 transition-transform duration-300" strokeWidth={2.5} />
                    </div>

                    {/* Destination Input */}
                    <div className="flex flex-col flex-1 min-w-[210px] group relative h-[100px] w-full lg:w-auto lg:pl-10">
                        <label className="text-[18px] font-bold text-slate-400 mb-2 block">Going To</label>
                        <input
                            type="text"
                            className="bg-transparent border-none outline-none font-bold text-slate-900 tracking-tighter text-[30px] p-0 placeholder:text-slate-300 leading-none h-[34px] w-full"
                        ...
                        <div className="absolute bottom-[-10px] left-0 w-full h-[1.5px] bg-slate-200 group-hover:bg-slate-300 hidden lg:block lg:ml-10" />
                    ...
                    <Popover open={isDepOpen} onOpenChange={setIsDepOpen}>
                        <PopoverTrigger asChild>
                            <div className="flex flex-col w-full lg:w-[170px] xl:w-[190px] group cursor-pointer relative h-[100px]">
                                <label className="text-[18px] font-bold text-slate-400 mb-2 flex items-center gap-1">Departure Date <ChevronDown className="w-4 h-4" /></label>
                                <div className="font-bold text-slate-900 tracking-tighter text-[30px] leading-none h-[34px] flex items-center">{date ? format(date, "dd MMM' yy") : "Select Date"}</div>
                                <p className="text-[15px] text-slate-500 mt-2 font-medium">{date ? format(date, "EEEE") : ""}</p>
                                <div className="absolute bottom-[-10px] left-0 w-full h-[1.5px] bg-slate-200 group-hover:bg-slate-300" />
                            </div>
                        </PopoverTrigger>
                        <PopoverContent className="w-auto p-0 bg-white rounded-3xl shadow-2xl border-none overflow-hidden" align="center">
                            <Calendar
                                mode="single"
                                selected={date}
                                onSelect={(d) => {
                    ...
                    {/* Return Date Calendar (Corrected & Functional) */}
                    <Popover open={isRetOpen} onOpenChange={setIsRetOpen}>
                        <PopoverTrigger asChild>
                            <div
                                className={cn(
                                    "flex flex-col w-full lg:w-[170px] xl:w-[190px] group cursor-pointer relative h-[100px]",
                            ...
                            >
                                <label className="text-[18px] font-bold text-slate-400 mb-2 block">Return Date</label>
                                <div className="h-[52px] flex items-start pt-[2px]">
                                    {returnDate && tripType !== 'one-way' ? (
                                        <div className="font-bold text-slate-900 tracking-tighter text-[30px] leading-none">
                                            {format(returnDate, "dd MMM' yy")}
                                            <p className="text-[15px] text-slate-500 mt-2 font-medium">{format(returnDate, "EEEE")}</p>
                                        </div>
                                    ) : (
                                        <span className="text-[#3b82f6] text-[17px] font-semibold leading-[1.3] tracking-tight">
                    ...
                    <Popover open={isTravellerOpen} onOpenChange={setIsTravellerOpen}>
                        <PopoverTrigger asChild>
                            <div className="flex flex-col w-full lg:w-[170px] xl:w-[190px] group cursor-pointer relative h-[100px]">
                                <label className="text-[18px] font-bold text-slate-400 mb-2 flex items-center gap-1">Travellers & Class <ChevronDown className="w-4 h-4" /></label>
                                <div className="font-bold text-slate-900 tracking-tighter text-[30px] leading-none h-[34px] flex items-center truncate">
                                    {travellers.adults + travellers.children + travellers.infants} Traveller
                                </div>
                                <p className="text-[15px] text-slate-500 mt-2 font-medium">{cabinClass}</p>
                                <div className="absolute bottom-[-10px] left-0 w-full h-[1.5px] bg-slate-200 group-hover:bg-slate-300" />
                            </div>
                        </PopoverTrigger>
                        <PopoverContent className="w-[320px] p-5 bg-white rounded-xl shadow-2xl border-none">
                    ...
                    <div className="flex items-center justify-center lg:justify-end h-auto lg:h-[88px] pb-2 lg:pl-5 w-full lg:w-auto mt-2 lg:mt-0">
                        <Button onClick={handleSearch} className="w-full lg:w-auto bg-brand hover:bg-brand/90 text-white rounded-full px-10 py-6 lg:py-7 h-[56px] lg:h-[60px] text-[18px] lg:text-[20px] font-bold shadow-md flex items-center justify-center gap-1.5 transition-transform active:scale-95">
                            Search <ArrowUpRight className="w-6 h-6" strokeWidth={2.5} />
                        </Button>
                    </div>
                </div>
                ...
                <div className="flex items-center space-x-2.5 mt-2">
                    <button id="nonStop" onClick={() => setNonStop(!nonStop)} className={cn("w-[18px] h-[18px] rounded-[4px] border flex items-center justify-center shadow-sm", nonStop ? "bg-brand border-brand text-white" : "border-slate-300 bg-white")}>
                        {nonStop && <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                    </button>
                    <label htmlFor="nonStop" className="text-[15px] font-bold text-slate-400 cursor-pointer pt-[2px]" onClick={() => setNonStop(!nonStop)}>Non-Stops Flights</label>
                </div>
