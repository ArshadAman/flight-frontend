                      ...
                      <div className="absolute right-0 mt-4 w-[240px] bg-white border-t-[4px] border-[#D60D26] rounded-xl shadow-xl z-50 p-2 animate-in fade-in duration-200">
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="flex flex-col">
                            <button
                              onClick={(e) => { e.preventDefault(); e.stopPropagation(); setIsProfileManagementExpanded(!isProfileManagementExpanded); }}
                            ...
                            {isProfileManagementExpanded && (
                              <div className="mt-1 flex flex-col pl-2">
                                {[
                                  { label: "Update Profile", href: "/sale/profile?tab=update-profile" },
                                  { label: "Manage User", href: "/sale/profile?tab=manage-user" },
                                  { label: "Create Sub User", href: "/sale/profile?tab=manage-user&action=create-sub-user" },
                                  { label: "Change Logo", href: "/sale/profile?tab=change-logo" },
                                  { label: "Manage Customer Profile", href: "/sale/profile?tab=customer-profile", isLast: true },
                                ].map((sub, idx) => (
                                  <Link
                                    key={idx}
                                    href={sub.href}
                                  ...
                                  >
                                    ...
                                    <span className="pl-1 truncate">{sub.label}</span>
                                  </Link>
                                ))}
                              </div>
                            )}
          ...
          {user ? (
            <div className="flex flex-col items-center space-y-4 bg-gray-50 rounded-2xl p-6">
              ...
              <div className="w-full flex flex-col gap-2 pt-3 border-t border-slate-200">
                ...
                {isProfileManagementExpanded && (
                  <div className="mt-1 flex flex-col pl-2 bg-white rounded-lg border border-slate-100 shadow-sm overflow-hidden">
                    {[
                      { label: "Update Profile", href: "/sale/profile?tab=update-profile" },
                      { label: "Manage User", href: "/sale/profile?tab=manage-user" },
                      { label: "Create Sub User", href: "/sale/profile?tab=manage-user&action=create-sub-user" },
                      ...
                      <Link
                        ...
                        href={sub.href}
                        onClick={() => setIsMobileMenuOpen(false)}
                        className={`flex items-center group/sub text-slate-600 hover:text-[#D60D26] hover:bg-slate-50 transition-colors text-[13px] font-bold py-3 px-3 ${!sub.isLast ? 'border-b border-slate-100' : ''}`}
                      >
                ...
                <Link href="/sale/profile?tab=notice-board" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-2 hover:bg-slate-100/50 py-2.5 px-3 rounded-lg transition-colors">
                  <CurvedArrowIcon />
                  <span className="font-bold text-slate-800 text-[14px]">Notice Board</span>
                </Link>

                <Link href="/sale/profile?tab=contact" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-2 hover:bg-slate-100/50 py-2.5 px-3 rounded-lg transition-colors">
                  <CurvedArrowIcon />
                  <span className="font-bold text-slate-800 text-[14px]">Contact</span>
                </Link>
              </div>
              ...
              <Button
                ...
                variant="outline"
                className="w-full mt-4 text-[16px] rounded-full py-5 font-[600] border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700"
              >
            ...
            <Button
                ...
                className="text-[16px] xl:text-[22px] rounded-full px-4 xl:px-6 py-[20px] xl:py-[24px] font-[700] tracking-wide shadow-none transition-transform hover:scale-105 active:scale-95 bg-brand text-white hover:bg-brand/90 w-full"
              >
                Login / Signup
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="ml-1.5 hidden xl:block"><path d="M7 17 17 7" /><path d="M7 7h10v10" /></svg>
            </Button>
